...
export default function GenericForm({ formConfig, urlSuffix, entites, setEntities, title, subtPath, beforeFinish = true, checkInputs = false, postEvent, buttonLabel }) {
	...
	const onSubmit = (e) => {
		...
		if (checkInputs && checkInputs(entites, currentRow)) {
			...
			if (Object.keys(formConfig).length > 1) {
				currentRow + 1 !== Object.keys(formConfig).length && checkInputs(entites, currentRow) ?
					setCurrentRow(currentRow + 1)
					:
					beforeFinish(entites) ? handleSubmit(urlSuffix, beforeFinish(entites), postEvent) : null
			} else {
									...
									{row.fields.map((field, index) =>
										field ? (
											<Form.Group className={`group`} controlId={`formBasic${field.id}`} key={index}>
												...
												/>
												{/* {field.bottomLabel ? (field.bottomLabel) : null} */}
